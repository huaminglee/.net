...
public partial class Approve_newApprove : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["type"] != null)
        {
            ...
            title.InnerText = "成都市水务局" + S_App.ShowTitle(ARCHIVE_TYPE) + "审批单";//设置标题
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(contentTask_Id.ToString()) || contentTask_Id == 0)
            {
                ...
                if (ARCHIVE_TYPE == "61" || ARCHIVE_TYPE == "62")
                {
                    Archive.ARCHIVE_CONTENT = AidHelp.FilterSpecial(txtRemark.Text);
                    Archive.IS_SHOW_IN_SZYD = "1";
                }
                ...
                if (ViewState["End_Flag"] != null && ViewState["End_Flag"].ToString() == "1")
                {
                    nPrompt("提交成功!", 2);
                }
                else if (result && (ViewState["CuurStemp_State"] == null || ViewState["CuurStemp_State"].ToString() == "0" || string.IsNullOrEmpty(ViewState["CuurStemp_State"].ToString())))
                {
                    contentTask_Id = (int)Task_Id;
                    this.Page.ClientScript.RegisterStartupScript(GetType(), "showDiv", "<script>window.location.href='/Archive/RecipientList.aspx?tId=" + contentTask_Id + "&nId=" + dplSkipList.SelectedValue + "'</script>");
                }
                else if (result && ViewState["CuurStemp_State"].ToString() == "1")
                {
                    contentTask_Id = (int)Task_Id;
                    this.Page.ClientScript.RegisterStartupScript(GetType(), "showDiv", "<script>window.location.href='/Archive/Is_ResponseUser.aspx?tId=" + contentTask_Id + "&nId=" + dplSkipList.SelectedValue + "'</script>");
                }
                else
                {
                    nPrompt("提交失败!", 0);
    ...
    {
        this.Page.ClientScript.RegisterStartupScript(GetType(), "showDiv", "<script>window.location.href='/Archive/RecipientList.aspx?tId=" + contentTask_Id + "&nId=" + dplSkipList.SelectedValue + "&action=copy'</script>");
    }
    /// <summary>
    /// 保存
    /// </summary>
