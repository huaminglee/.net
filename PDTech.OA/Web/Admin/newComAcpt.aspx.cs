...
public partial class Admin_newComAcpt : System.Web.UI.Page
{
    ...
    public void InitBindData()
    {
        ...
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> ApplyList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "竣工验收申请").ToList();
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> AccepList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "竣工验收鉴定书").ToList();
        ...
        {
            rpt_CompList.DataSource = ApplyList;
            rpt_CompList.DataBind();
            tr_showApplyList.Visible = true;
            hidApply.Value = "1";
        }
        else
        {
            tr_showApplyList.Visible = false;
            hidApply.Value = "0";
        }
        if (AccepList.Count > 0)
        {
            rpt_AccepList.DataSource = AccepList;
            rpt_AccepList.DataBind();
            tr_AccepList.Visible = true;
            hidAccep.Value = "1";
        }
        else
        {
            tr_AccepList.Visible = false;
            hidAccep.Value = "0";
        }
        #endregion
    }
    ...
    protected void rpt_CompList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
    ...
    {
        if (!string.IsNullOrEmpty(this.fileApply.PostedFile.FileName))
            UpLoadFile(this.fileApply, "竣工验收申请");
        if (!string.IsNullOrEmpty(this.fileAccep.PostedFile.FileName))
            UpLoadFile(this.fileAccep, "竣工验收鉴定书");
        uBindData();
        InitBindData();
    }
    /// <summary>
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            where.REMARK = AidHelp.FilterSpecial(txtRemark.Text);
    ...
    protected void rpt_AccepList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            PDTech.OA.Model.OA_ATTACHMENT_FILE item = e.Item.DataItem as PDTech.OA.Model.OA_ATTACHMENT_FILE;
