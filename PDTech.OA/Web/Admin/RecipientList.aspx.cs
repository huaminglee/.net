...
public partial class Admin_RecipientList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (task != null)
            {
                hidcurstepid.Value = task.CURRENT_STEP_ID.ToString();
                if (!nextisend&& projectid != "" && int.Parse(content_nextstepId) > int.Parse(hidcurstepid.Value))
                {
                    hidischuzhang.Value = "1";//提交至下一步 只显示处长
                }
                else if (int.Parse(content_nextstepId) == int.Parse(hidcurstepid.Value) &&projectid != "")
                {
                    hidischuzhang.Value = "2";//内部循环 只显示本部门人员
                }
                //判断是否需要签名
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (string.IsNullOrEmpty(content_operType) || content_operType == "Handle")
            {
                ...
                if (result == 1)
                {
                    #region 处理SW_PROJECT_STEP表
                    if (projectid!=""&& int.Parse(content_nextstepId) > int.Parse(hidcurstepid.Value))
                    {
                        stepBll.UPsetpinfo(int.Parse(projectid), int.Parse(hidcurstepid.Value), hidUserList.Value.TrimEnd(','), CurrentAccount.USER_ID.ToString(), Convert.ToDateTime(txtInComingDate.Text), Convert.ToDateTime(txtInComingDate.Text).AddHours(Convert.ToDouble(ddlRemindDate.SelectedValue)));
                    }
                    else if (projectid!=""&&int.Parse(content_nextstepId) == int.Parse(hidcurstepid.Value))
                    {
                        stepBll.uplimittime(int.Parse(projectid), int.Parse(hidcurstepid.Value), Convert.ToDateTime(txtInComingDate.Text), Convert.ToDateTime(txtInComingDate.Text).AddHours(Convert.ToDouble(ddlRemindDate.SelectedValue)), hidUserList.Value.TrimEnd(','));
                    }
