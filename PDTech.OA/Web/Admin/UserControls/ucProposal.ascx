<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ucProposal.ascx.cs" Inherits="Admin_UserControls_ucProposal" %>
...
<section class="container">
    <div class="container-fluid">
        <fieldset>
            <legend><small class="text-primary">建议提案</small></legend>
            <!-- 查询条件 -->
            <div class="row">
                <div class="col-sm-3">
                                ...
                                <Zore:Repeater ID="rpt_taskList" runat="server" OnItemDataBound="rpt_taskList_ItemDataBound" OnItemCommand="rpt_taskList_ItemCommand">
                                    ...
                                    <ItemTemplate>
                                        <tr>
                                            ...
                                            <td>
                                                <asp:Image runat="server" Height="16" ID="imgStatus" ImageUrl='<%# Eval("ATTACHMENT_COUNT").ToString() == "0" ? "~/images/none.png" : "~/images/mail-attachment.png" %>' />
                                            </td>
...
<script>
    ...
    function NewArchive() {
        $.layer({
            type: 2,
            title: '新建提案',
            iframe: { src: '/Admin/newProposal.aspx' },
            maxmin: true,
            area: ['985px', '585px'],
    ...
    function doWork(tID, arId) {
        $.layer({
            type: 2,
            title: '办理提案',
            iframe: { src: '/Admin/newProposal.aspx?tId=' + tID + '&arId=' + arId },
            maxmin: true,
            area: ['985px', '585px'],
            border: [1, 0.2, '#000', true],
            offset: ['20px', '']
    ...
    function selWork(tID, arId) {
        $.layer({
            type: 2,
            title: '查看提案',
            iframe: { src: '/Admin/selProposal.aspx?tId=' + tID + '&arId=' + arId },
            maxmin: true,
            area: ['985px', '585px'],
            border: [1, 0.2, '#000', true],
            offset: ['20px', '']
