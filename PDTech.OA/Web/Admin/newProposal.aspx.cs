...
public partial class Admin_newProposal : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                dplDateDocNo.Items.Add(new ListItem((DateTime.Now.Year - i).ToString(), (DateTime.Now.Year - i).ToString()));
            }
        ...
        if (tWhere != null)
        {
            ...
            if (attList.Count > 0)
            {
                foreach (var item in attList)
                {
                    ...
                    {
                        case "PeriodNum":
                            txtPeriodNum.Value = item.VALUE;
                            break;
                        case "Header":
                            txtHeader.Value = item.VALUE;
                            break;
                        case "UserNum":
                            txtUserNum.Value = item.VALUE;
                            break;
                        case "Summary":
                            txtSummary.Text = item.VALUE;
                            break;
                        case "Lordsent":
            ...
            dplDateDocNo.SelectedValue = Arwhere.ARCHIVE_NO;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(contentTask_Id.ToString()) || contentTask_Id == 0)
            {
                ...
                Archive.ARCHIVE_NO = dplDateDocNo.SelectedValue;
                ...
                if (!string.IsNullOrEmpty(txtPeriodNum.Value))
                {
                    attList.Add(new PDTech.OA.Model.ATTRIBUTES() { KEY = "PeriodNum", VALUE = new ConvertHelper(txtPeriodNum.Value.Trim()).String });
                }
                if (!string.IsNullOrEmpty(txtHeader.Value))
                {
                    attList.Add(new PDTech.OA.Model.ATTRIBUTES() { KEY = "Header", VALUE = new ConvertHelper(txtHeader.Value.Trim()).String });
                }
                if (!string.IsNullOrEmpty(txtUserNum.Value))
                {
                    attList.Add(new PDTech.OA.Model.ATTRIBUTES() { KEY = "UserNum", VALUE = new ConvertHelper(txtUserNum.Value.Trim()).String });
                }
                if (!string.IsNullOrEmpty(txtSummary.Text))
                {
                    attList.Add(new PDTech.OA.Model.ATTRIBUTES() { KEY = "Summary", VALUE = new ConvertHelper(txtSummary.Text.Trim()).String });
                }
                if (!string.IsNullOrEmpty(hidAttachmentIds.Value))
                {
