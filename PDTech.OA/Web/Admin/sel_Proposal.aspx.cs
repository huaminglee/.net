...
public partial class Admin_sel_Proposal : System.Web.UI.Page
{
    ...
    public void onBindData()
    {
        ...
        if (attList.Count > 0)
        {
            foreach (var item in attList)
            {
                ...
                {
                    case "PeriodNum":
                        txtPeriodNum.Text = item.VALUE;
                        break;
                    case "Header":
                        txtHeader.Text = item.VALUE;
                        break;
                    case "UserNum":
                        txtUserNum.Text = item.VALUE;
                        break;
                    case "Summary":
        ...
        dplDateDocNo.Text = Arwhere.ARCHIVE_NO;//所属年份
        txtRemark.Text = Arwhere.ARCHIVE_CONTENT;//备注
        txtTitle.Text = Arwhere.ARCHIVE_TITLE;//标题
        dplIsSecret.Text =Enum.Parse(typeof(EArchiveUrgency),Arwhere.IS_SECRET.ToString(),true).ToString();
        dplUrgency.Text = Enum.Parse(typeof(EArchiveUrgency), Arwhere.PRI_LEVEL.ToString()).ToString();
        #endregion
        ...
        where.REF_ID = Arwhere.ARCHIVE_ID;
