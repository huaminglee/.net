<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RecipientList.aspx.cs" Inherits="Admin_RecipientList" %>
    ...
    <form id="rForm" runat="server">
        <div class="Main_Body">
                ...
                <div class="con_say">
                    ...
                    <fieldset>
                        ...
                        <div class="con_idea">
                            ...
                            <textarea runat="server" id="txtIdea" style="resize: none;" class="txtArea"></textarea>
                        </div>
                    </fieldset>
                </div>
                <div class="con_Optionw">
                    <div id="Option_setting" class="con_option_box">
                        ...
                        <div class="chgDate left" runat="server" id="deadline">
                            <span class="RemindDate">办理时限<asp:TextBox ID="txtInComingDate" runat="server" CssClass="inputx Wdate input152" onFocus="WdatePicker({isShowClear:false,readOnly:true})"></asp:TextBox></span>
                        </div>
                        ...
                        <div class="RemindObject left" runat="server" id="TipsDate">
                            <span class="RemindDate">提醒对象：
                                <asp:DropDownList ID="ddlRemindUser" runat="server" CssClass="ddlinput input152"></asp:DropDownList>
                            </span>
                            <br />
                            <span class="RemindDate">提醒时间：
                                <asp:DropDownList ID="ddlRemindDate" runat="server" CssClass="ddlinput input152"></asp:DropDownList>
                            </span>
                        </div>
                    </div>
                    <fieldset>
            ...
            <div class="con_btn">
                ...
                <asp:HiddenField ID="hidischuzhang" Value="0" runat="server" />
            </div>
        </div>
        <script>
            ...
            $(function () {
                $("#tt").tree({
                        ...
                        if (Is_Multiplayer == "1" || $("#hidAction").val() == "copy") {
                            if (node.status == "person" && !contains(valArr, node.id)) {
                                if (!contains(valArr, node.id))
                                    valArr.push(node.id);
                                /***进行分配***/
                                $("#yxry").append("<a class='btn-default btn-block'>" + node.text + "<button id='" + node.id + "' name='" + node.status + "' type='button' class='close' title='关闭' onclick='mbtnClose(this);'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button></a>");
                            }
                            if (node.status == "department" && node.duid.length > 0 && !contains(valArr, node.duid)) {
                                if (!contains(valArr, node.duid))
                                    valArr.push(node.duid);
                                /***进行分配***/
                                $("#yxry").append("<a class='btn-default btn-block'>" + node.text + "<button id='" + node.duid + "' name='" + node.status + "' type='button' class='close' title='关闭' onclick='mbtnClose(this);'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button></a>");
                            }
                    ...
                    method: 'get',
                    url: "/Ajax/Public_Tree.ashx?pageState=gwry&txt_name=" + encodeURIComponent($.trim($("#txtPersonName").val())) + "&action=" + $("#hidAction").val() + "&nId=" + $("#hidCuurentStepId").val() + "&ischuzhang=" + $("#hidischuzhang").val() + ""
                });
                ...
                $("#btnSearch").click(function () {
                    $("#tt").tree({
                        method: 'get',
                        url: "/Ajax/Public_Tree.ashx?pageState=gwry&txt_name=" + encodeURIComponent($("#txtPersonName").val()) + "&action=" + $("#hidAction").val() + "&nId=" + $("#hidCuurentStepId").val() + "&ischuzhang=" + $("#hidischuzhang").val() + ""
                    });
                    $("#yxry").html("");
                });
            ...
            function saveTemp() {
                if ($("#hidUserList").val().length > 0) {
                    $.layer({
                        type: 2,
                        title: '新增模板',
                        iframe: { src: '/Admin/AddTemp.aspx?uIds=' + $("#hidUserList").val() },
                        maxmin: true,
                        area: ['410px', '130px'],
            ...
            function showTempList() {
                $.layer({
                    type: 2,
                    title: '接收人员模板',
                    iframe: { src: '/Admin/selTempList.aspx?code=' + $("#hidCode").val() + "" },
                    maxmin: true,
                    area: ['510px', '530px'],
                    border: [1, 0.2, '#000', true],
                    offset: ['20px', '']
