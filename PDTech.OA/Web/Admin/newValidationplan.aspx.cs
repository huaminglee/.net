...
public partial class Admin_newValidationplan : System.Web.UI.Page
{
    ...
    public void InitBindData()
    {
        ...
        ///获取步骤审查意见附件
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> EvaList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "申报项目评审").ToList();
        ///获取步骤审查意见附件
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> exList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "申报项目签审").ToList();
        ...
        if (exList.Count > 0)
        {
            rpt_ProExetList.DataSource = exList;
            rpt_ProExetList.DataBind();
            trProExetList.Visible = true;
            hidProExet.Value = "1";
        }
        else
        {
            trProExetList.Visible = false;
            hidProExet.Value = "0";
        }
        if (EvaList.Count > 0)
        {
            rpt_ProEvaList.DataSource = EvaList;
            rpt_ProEvaList.DataBind();
            tr_showProEvaList.Visible = true;
            hidProEva.Value = "1";
        }
        else
        {
            tr_showProEvaList.Visible = false;
            hidProEva.Value = "0";
        }
        #endregion
        #endregion
    ...
    protected void rpt_ProEvaList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UpLoadFile(this.fileProEva, "申报项目评审");
            UpLoadFile(this.fileProExet, "申报项目签审");
            bool result = false;
            if (hidstate.Value == "0")
            {
    ...
    {
        if (!string.IsNullOrEmpty(this.fileProEva.PostedFile.FileName))
            UpLoadFile(this.fileProEva, "申报项目评审");
        if (!string.IsNullOrEmpty(this.fileProExet.PostedFile.FileName))
            UpLoadFile(this.fileProExet, "申报项目签审");
        uBindData();
        InitBindData();
    }
    protected void rpt_ProExetList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            PDTech.OA.Model.OA_ATTACHMENT_FILE item = e.Item.DataItem as PDTech.OA.Model.OA_ATTACHMENT_FILE;
