...
public partial class Admin_newDisbursementfunds : System.Web.UI.Page
{
    ...
    public void InitBindData()
    {
        ...
        ///获取步骤付款申请附件
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> PayList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "付款申请").ToList();
        ///获取步骤付款申请审核结果附件
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> payrList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "付款申请审核结果").ToList();
        ///获取步骤资金拨付签审附件
        IList<PDTech.OA.Model.OA_ATTACHMENT_FILE> MoneyList = allList.Where(o => !string.IsNullOrEmpty(o.DATA1) && o.DATA1 == "资金拨付签审").ToList();
        ...
        if (PayList.Count > 0)
        {
            rpt_PayList.DataSource = PayList;
            rpt_PayList.DataBind();
            hidbidPay.Value = "1";
            tr_ShowPayList.Visible = true;
        }
        else
        {
            hidbidPay.Value = "0";
            tr_ShowPayList.Visible = false;
        }
        if (payrList.Count > 0)
        {
            rpt_PayrList.DataSource = payrList;
            rpt_PayrList.DataBind();
            hidPayr.Value = "1";
            tr_ShowPayrList.Visible = true;
        }
        else
        {
            hidPayr.Value = "0";
            tr_ShowPayrList.Visible = false;
        }
        if (MoneyList.Count > 0)
        {
            rpt_MoneyList.DataSource = MoneyList;
            rpt_MoneyList.DataBind();
            tr_ShowMoney.Visible = true;
        }
        else
        {
            tr_ShowMoney.Visible = false;
        }
        #endregion
        #endregion
    }
    ...
    #region 附件处理
    protected void rpt_PayList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
    ...
    protected void rpt_PayrList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            PDTech.OA.Model.OA_ATTACHMENT_FILE item = e.Item.DataItem as PDTech.OA.Model.OA_ATTACHMENT_FILE;
    ...
    protected void rpt_MoneyList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            PDTech.OA.Model.OA_ATTACHMENT_FILE item = e.Item.DataItem as PDTech.OA.Model.OA_ATTACHMENT_FILE;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UpLoadFile(this.filePay, "付款申请");
            UpLoadFile(this.filePayr, "付款申请审核结果");
            UpLoadFile(this.fileMoney, "资金拨付签审");
            bool result = false;
            if (hidstate.Value == "0")
            {
    ...
    {
        if (!string.IsNullOrEmpty(this.filePay.PostedFile.FileName))
            UpLoadFile(this.filePay, "付款申请");
        if (!string.IsNullOrEmpty(this.filePayr.PostedFile.FileName))
            UpLoadFile(this.filePayr, "付款申请审核结果");
        if (!string.IsNullOrEmpty(this.fileMoney.PostedFile.FileName))
            UpLoadFile(this.fileMoney, "资金拨付签审");
        uBindData();
        InitBindData();
    }
