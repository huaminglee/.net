<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ucUnitDocList.ascx.cs" Inherits="Archive_UserControls_ucUnitDocList" %>
...
<section class="container">
    <div class="container-fluid">
        <fieldset>
            <legend><small class="text-primary">单位发文</small></legend>
            <!-- 查询条件 -->
            <div class="row">
                <div class="col-sm-3">
                ...
                <div class="col-sm-3">
                    <div class="input-group">
                        ...
                        <asp:DropDownList ID="dplStatus" runat="server" CssClass="easyui-combobox" Height="35px" Width="150px" data-options="panelHeight:'auto'">
                            ...
                            <asp:ListItem Value="0" Selected="True" Text="待处理工作"></asp:ListItem>
                            <asp:ListItem Value="1" Text="已处理工作"></asp:ListItem>
                        </asp:DropDownList>
                    </div>
                </div>
                <div class="col-sm-1">
                                    ...
                                    <ItemTemplate>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a onclick='doWork(<%# Eval("ARCHIVE_TASK_ID") %>,<%# Eval("ARCHIVE_ID") %>)'>办理</a> |</asp:Label>
                                                <a onclick='selWork(<%# Eval("ARCHIVE_TASK_ID") %>,<%# Eval("ARCHIVE_ID") %>)'>查看</a>|
...
<script>
    ...
    function NewArchive() {
        $.layer({
            type: 2,
            title: '新建公文',
            iframe: { src: '/Archive/NewUnitDoc.aspx' },
            maxmin: true,
            area: ['985px', '585px'],
    ...
    function doWork(tID, arId) {
        $.layer({
            type: 2,
            title: '办理公文',
            iframe: { src: '/Archive/NewUnitDoc.aspx?tId=' + tID + '&arId=' + arId },
            maxmin: true,
            area: ['985px', '585px'],
            border: [1, 0.2, '#000', true],
            offset: ['20px', '']
    ...
    function selWork(tID, arId) {
        $.layer({
            type: 2,
            title: '查看公文',
            iframe: { src: '/Archive/selUnitDoc.aspx?tId=' + tID + '&arId=' + arId },
            maxmin: true,
            area: ['985px', '585px'],
            border: [1, 0.2, '#000', true],
            offset: ['20px', '']
