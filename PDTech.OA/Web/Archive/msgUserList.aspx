<%@ Page Language="C#" AutoEventWireup="true" CodeFile="msgUserList.aspx.cs" Inherits="Archive_msgUserList" %>
        ...
        <script>
            ...
            $(function () {
                    ...
                    onClick: function (node) {
                        if (node.status == "person" && !contains(valArr, node.id)) {
                            if (!contains(valArr, node.id))
                                valArr.push(node.id);
                            if (!contains(returnVal, node.text))
                                returnVal.push(node.text);
                            /***进行分配***/
                ...
                $("#btnSearch").click(function () {
                    $("#tt").tree({
                        method: 'get',
                        url: "/Ajax/Public_Tree.ashx?pageState=gwry&txt_name=" + encodeURIComponent($("#txtPersonName").val()) + "&action=Msg" + ""
                    });
                    $("#yxry").html("");
                });
            })
            ...
            function OnOK() {
                window.parent.setreceiverUser( $("#hidUserList").val(),$("#hidName").val());
            }
