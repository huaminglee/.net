...
public partial class Archive_sel_UnitDoc : System.Web.UI.Page
{
    ...
    public void onBindData()
    {
        ...
        if (attList.Count > 0)
        {
            foreach (var item in attList)
            {
                ...
                {
                    case "ArCreateUser":
                        lbUserName.Text = item.VALUE;
                        break;
                    case "ArIsNet":
                        txtIs_net.Text = item.VALUE;
                        break;
                    case "ArIsSpe":
                        txtIs_Spe.Text = item.VALUE;
                        break;
                    case "ArPrintNum":
                        txtPrintNum.Text = item.VALUE;
                        break;
                    case "ArProo":
                        txtProo.Text = item.VALUE;
                        break;
                    case "ArReview":
                        txtReview.Text = item.VALUE;
                        break;
                    case "ArCopies":
                        txtCopies.Text = item.VALUE;
                        break;
                    case "ArLordsent":
        ...
        lbDocTitle.Text = "" + Arwhere.ARCHIVE_NO + "号";
        txtRemark.Text = Arwhere.ARCHIVE_CONTENT;
        txtTitle.Text = Arwhere.ARCHIVE_TITLE;
        dplIsSecret.Text = new ConvertHelper(Enum.Parse(typeof(EArchiveIsSecret), Arwhere.IS_SECRET.ToString())).String;
