...
public partial class SysManege_NewLingual : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (!string.IsNullOrEmpty(sysID) && AidHelp.IsDecimal(sysID))
            { BindData(); }
        }
    }
    /// <summary>
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            this.Page.ClientScript.RegisterStartupScript(GetType(), "showDiv", "<script>window.parent.layer.alert('文种名称不能为空',8)</script>");
            return;
        }
