...
public partial class Risk_newArRisk : System.Web.UI.Page
{
    ...
    public string t_rand = "";//页面JS或者CSS的时间戳
    public decimal ARCHIVE_TYPE = 51;//公文的类型ID
    public decimal ptype = 0;//生成处置单的对象类型
    public decimal p_Id = 0;//生成处置单的公文ID
    public decimal next_step = 0;//下一步ID
    public int contentTask_Id = 0;//当前处置单任务ID
    public decimal arId = 0;//当前风险处置公文ID
    public decimal rId = 0;//监察室提醒信息ID
    public decimal rmID = 0;
    public decimal uID = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            rmID = decimal.Parse(Request.QueryString["rmID"].ToString());
        }
        if (Request.QueryString["uid"] != null)
        {
            uID = decimal.Parse(Request.QueryString["uid"].ToString());
        }
        if (Request.QueryString["p_Id"] != null)//获取当前公文参数
        {
            p_Id = decimal.Parse(Request.QueryString["p_Id"].ToString());
        }
    ...
    public void InitLoad()
    {
        ...
        if (rmID == 20 && CurrentAccount.ISHavePurview("leader"))
            dplRiskList.SelectedValue = "监督督查";
        else if (rmID == 10)
        ...
        if (uID != 0)
        {
            ...
            PDTech.OA.Model.USER_INFO uInfo = uBll.GetUserInfo(uID);
            if (uInfo != null)
            {
                txtUserName.Value = uInfo.FULL_NAME;
        ...
        if (ptype != 0 && p_Id != 0)
        {
            ...
            {
                PDTech.OA.BLL.VIEW_PROJECT_STEP_USER pBll = new PDTech.OA.BLL.VIEW_PROJECT_STEP_USER();
                PDTech.OA.Model.VIEW_PROJECT_STEP_USER sInfo = pBll.get_paging_project_info(new PDTech.OA.Model.VIEW_PROJECT_STEP_USER() { PROJECT_ID = p_Id });
                if (sInfo != null)
                {
                    txtBusinessNum.Text = sInfo.PROJECT_ID.ToString();
                    txtBusinessNum.Enabled = false;
                    txtHandleDate.Value = sInfo.START_TIME.ToString();
                    txtHandleDate.Disabled = true;
                    dplDeptList.SelectedValue = sInfo.TOP_RESPONSE_DEPT.ToString();
            ...
            {
                PDTech.OA.BLL.OA_ARCHIVE currBll = new PDTech.OA.BLL.OA_ARCHIVE();
                PDTech.OA.Model.VIEW_ARCHIVE_USER_DEPT tInfo = currBll.get_ArchiveAll_UserList(new PDTech.OA.Model.VIEW_ARCHIVE_USER_DEPT() { ARCHIVE_ID = p_Id });
                if (tInfo != null)
                {
                    txtBusinessNum.Text = tInfo.ARCHIVE_ID.ToString();
                    txtBusinessNum.Enabled = false;
                    txtHandleDate.Value = tInfo.CREATE_TIME.ToString();
                    txtHandleDate.Disabled = true;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(contentTask_Id.ToString()) || contentTask_Id == 0)
        {
            ...
            bool result = ArchiveBll.AddRisk(Archive, CurrentAccount.ClientHostName, CurrentAccount.ClientIP, fjIds, attList, (int)p_Id, 0, (int)ptype, (int)rId, out Archive_Id, out Task_Id);
            if (result)
            {
    ...
    protected void btnSearchList_Click(object sernder, EventArgs e)
    {
        BindData();
    }
