<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ExpireWater.aspx.cs" Inherits="Monitor_ExpireWater" %>
<!DOCTYPE html>
...
<body>
    ...
    <section class="container">
        <div class="container-fluid">
            <fieldset>
                <legend><small class="text-primary">超期预警--水务项目</small></legend>
                <div class="row">
                    <div class="col-sm-3">
    ...
    <script>
        ...
        $(function () {
            $.get("/Ajax/PublicQuery.ashx", { pageState: "expireWater", username: $("#hiduserid").val(), pageSize: pageSize },
              function (data) {
                  ...
                  if (totalNum > 0) {
                      ...
                      $.each(obj, function (i) {
                          ...
                          //    strTable += "<tr class='danger'><td>" + obj[i].rowno + "</td><td title='" + obj[i].project_name + "'>" + obj[i].project_name.substring(0, 15) + "</td><td>" + obj[i].file_dept_name + "</td><td>" + obj[i].creator + "</td><td>" + obj[i].owner + "</td><td>" + obj[i].owner_dept + "</td><td>" + obj[i].start_time + "</td><td>" + obj[i].step_name + "</td><td>" + obj[i].limit_time + "</td><td>" + (obj[i].risk_handle_archive_id == "" ? "<span class='glyphicon glyphicon-file' title='生成风险处置单' onclick='createRiskHandle(" + obj[i].project_id + "," + obj[i].project_step_id + ",33);'>(生成风险处置单)</span>" : "<span class='glyphicon glyphicon-file' title='查看风险处置单' onclick='showRiskHandle(" + obj[i].risk_handle_archive_id + ");'>(已处理)</span>") + "</td><td class='text-center'><span class='glyphicon glyphicon-search' title='查看' onclick='showDetail(" + obj[i].project_id + ")'></span></td></tr>";
                          //}
                          //else {
                              strTable += "<tr class='warning'><td>" + obj[i].rowno + "</td><td title='" + obj[i].project_name + "'>" + obj[i].project_name.substring(0, 15) + "</td><td>" + obj[i].file_dept_name + "</td><td>" + obj[i].creator + "</td><td>" + obj[i].owner + "</td><td>" + obj[i].owner_dept + "</td><td>" + obj[i].start_time + "</td><td>" + obj[i].step_name + "</td><td>" + obj[i].limit_time + "</td><td>" + (obj[i].risk_handle_archive_id == "" ? "<span class='glyphicon glyphicon-file' title='生成风险处置单' onclick='createRiskHandle(" + obj[i].project_id + "," + obj[i].project_step_id + ",33);'>(生成风险处置单)</span>" : "<span class='glyphicon glyphicon-file' title='查看风险处置单' onclick='showRiskHandle(" + obj[i].risk_handle_archive_id + ");'>(已处理)</span>") + "</td><td class='text-center'><span class='glyphicon glyphicon-search' title='查看' onclick='showDetail(" + obj[i].project_id + ")'></span></td></tr>";
                          //}
                         
                      });
                      $("#tb_expireWater").html(strTable);
                      strTable = "";
                      pagination();//分页
                  }
                  else {
                ...
                $.get("/Ajax/PublicQuery.ashx", {
                  ...
                  function (data) {
                      ...
                      if (totalNum > 0) {
                          obj = $.parseJSON(objTemp.DataSources);//转换出数据源
                          $.each(obj, function (i) {
                              strTable += "<tr><td>" + obj[i].rowno + "</td><td title='" + obj[i].project_name + "'>" + obj[i].project_name.substring(0, 15) + "</td><td>" + obj[i].file_dept_name + "</td><td>" + obj[i].creator + "</td><td>" + obj[i].owner + "</td><td>" + obj[i].start_time + "</td><td>" + obj[i].step_name + "</td><td>" + obj[i].limit_time + "</td><td>" + (obj[i].risk_handle_archive_id == "" ? "<span class='glyphicon glyphicon-file' title='生成风险处置单' onclick='createRiskHandle(" + obj[i].project_id + "," + obj[i].project_step_id + ",33);'>(生成风险处置单)</span>" : "<span class='glyphicon glyphicon-file' title='查看风险处置单' onclick='showRiskHandle(" + obj[i].risk_handle_archive_id + ");'>(已处理)</span>") + "</td><td class='text-center'><span class='glyphicon glyphicon-search' title='查看' onclick='showDetail(" + obj[i].project_id + ")'></span></td></tr>";
                          });
                          $("#tb_expireWater").html(strTable);
                          strTable = "";
                          pagination();//分页
