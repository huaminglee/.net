...
tab.panel("options").tab.remove(); //wqy Tab完全释放内存
var frame = $('iframe', tab); if (frame.length > 0) {
    // frame[0].contentWindow.document.write('');
    frame[0].contentWindow.close(); frame.remove(); if ($.browser.msie) { CollectGarbage(); }
}
tab.panel("destroy");
