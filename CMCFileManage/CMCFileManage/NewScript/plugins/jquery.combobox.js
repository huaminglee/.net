...
//wqy
function _CtrlDown(combobox_object) {
    var ThisOptions = $.data(combobox_object, "combobox").options;
    var multiple = ThisOptions.multiple;
    var onlySelectOne = ThisOptions.onlySelectOne;
    if (multiple || !onlySelectOne) {
        $.data(combobox_object, "combobox").options.multiple = true;
        $.data(combobox_object, "combobox").options.onlySelectOne = false;
    }
}
//wqy
function _CtrlUp(combobox_object) {
    if ($.data(combobox_object, "combobox").options.onlySelectOne == false) {
        $.data(combobox_object, "combobox").options.multiple = false;
    }
...
//wqy 当不是多选时关闭下拉框
if (_30.onlySelectOne != false) {
    $("div.combo-panel").panel("close");
}
});
//wqy
_31.keydown(function(e) {
    switch (e.keyCode) {
        case 17:
            _30.keyHandler.CtrlDown.call(_2d);
            break;
        default:
    }
});
_31.keyup(function(e) {
    switch (e.keyCode) {
        case 17:
            _30.keyHandler.CtrlUp.call(_2d);
            break;
        default:
    }
});
