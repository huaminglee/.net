<!--
document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 157; height: 182; display: none'>");
...
function writeIframe()
{
    ...
    "table#tableMain{ width: 154; height: 180;}"+
    ...
    strIframe += "<select name=tmpYearSelect  onblur='parent.hiddenSelect(this)' style='z-index:1;position:absolute;top:3;left:28;width=60;display:none'"+
    ...
    " onchange='parent.WebCalendar.thisMonth=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
     "<table id=tableMain class=bg border=0 cellspacing=2 cellpadding=0 width='100%' height='100%'>"+
    "<tr><td width=154 height=19 bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table width=154 id=tableHeadborder=0 cellspacing=1 cellpadding=0 align='center'><tr align=center>"+
    ...
    "</tr></table>"+
    "</td></tr><tr><td height=20 align='center'><table id=tableWeek border=1 width=154 cellpadding=0 cellspacing=0 ";
    ...
    "</td></tr><tr><td valign=top width=154 bgcolor='"+ WebCalendar.lightColor +"'>"+
     "    <table id=tableDay height='100%' width=154 border=0 cellspacing=1 cellpadding=0>";
         for(var x=0; x<5; x++){ strIframe += "<tr>";
         for(var y=0; y<7; y++)  strIframe += "<td class=out id='meizzDay"+ (x*7+y) +"'></td>"; strIframe += "</tr>";}
         strIframe += "<tr>";
         for(var x=35; x<39; x++) strIframe += "<td class=out id='meizzDay"+ x +"'></td>";
         strIframe +="<td colspan=3 title='关闭'><input style=' background-color: "+
    ...
    "</td></tr>";
    if (WebCalendar.timeShow){strIframe+="<tr><td height=20 width=154  align='center' bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table width=154  border=0 cellspacing=1 cellpadding=0 align='center'><tr align=center>"+
    ...
    "        <td width=80 colspan=2 align='center' id='TdHour'  title='点击此处輸入時間'><input onblur='parent.InitHourAndMintue()' type='text' style='WIDTH: 60px' id='HourAndMinute'></td>"+
...
function calendar(IsHour) //主调函数
{
	WebCalendar.timeShow = IsHour;
    var e = window.event.srcElement;   writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = e;
 if (arguments.length == 1) WebCalendar.objExport = e;
    else WebCalendar.objExport = eval(arguments[1]);
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    if (WebCalendar.timeShow)
    {
		WebCalendar.iframe.TdHour.children(0).value=appendZero(parseInt(h, 10)) + ":" + appendZero(parseInt(mi, 10));
    }
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.timeShow)
        {
            var h = WebCalendar.thisHour, m = WebCalendar.thisMinute;
...
function InitHourAndMintue()
{
	var InputReg=/^([01]\d|2[0-3]):([0-5]\d)$/;
	var Result=WebCalendar.iframe.TdHour.children(0).value.match(InputReg);
	 if(Result==null)
	 {
		 alert("你设定的時間输出格式不对！\r\n\r\n请重新定义！"); return false;
	 }
	 ...
	 {
		WebCalendar.thisHour= parseInt(Result[1], 10);
		WebCalendar.thisMinute= parseInt(Result[2], 10);
	 }
}
//-->