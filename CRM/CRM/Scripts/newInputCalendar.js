 ...
 *说明：
 * 默认返回的日期时间格式如同：2006-09-23 08:08
 */
//------------------ 样式定义 ---------------------------//
//功能按钮同样样式
...
//年选择下拉框
var s_tiannet_select = "width:65px;display:none;";
//月、时、分选择下拉框
var s_tiannet_select2 = "width:48px;display:none;";
//日期选择控件体的样式
var s_tiannet_body = "width:175;background-color:#D3D3D3;display:none;z-index:999999;position:absolute;" +
  "border-left:1 solid #CCCCCC;border-top:1 solid #CCCCCC;border-right:1 solid #999999;border-bottom:1 solid #999999;";
//显示日的td的样式
var s_tiannet_day = "width:30px;height:20px;background-color:#D8F0FC;font-size:10pt;";
//字体样式
var s_tiannet_font = "color:#000000;font-size:10pt;cursor:hand;";
//链接的样式
...
//------------------ 变量定义 ---------------------------//
//var tiannetYearSt = 2005;//可选择的开始年份
//var tiannetYearEnd = 2016;//可选择的结束年份
var tiannetDateNow = new Date();
var tiannetYearSt=tiannetDateNow.getFullYear()-7;
var tiannetYearEnd=tiannetDateNow.getFullYear()+3;
...
document.write('<div align="center" id="divTiannetTurn" style="padding-top:2px;border:0;" Author="tiannet">');
document.write('</div>');
document.write('<div style="'+s_tiannet_line+'"></div>');
document.write('<div align="center" id="divTiannetDateText" Author="tiannet" style="padding-top:2px;width:165x">');
...
document.write(' <tr style="background-color:#D3D3D3;font-size:10pt;color:white;height:22px;" Author="tiannet">');
for(var i =0;i < weekName.length;i ++){
 //输出星期
 document.write('<td width="30" align="center" Author="tiannet">' + weekName[i] + '</td>');
}
document.write(' </tr>');
...
function tiannetPrevYear()  { 
 ...
 else { alert("年的范圍不超出該邊界（1000-9999）！");}
...
function tiannetNextYear()  { 
 if(tiannetYear > 999 && tiannetYear <10000){tiannetYear++;}
 else { alert("年的范圍不超出該邊界（1000-9999）！");return;}
 tiannetSetDay(tiannetYear,tiannetMonth);
...
function tiannetSetValue(bolHideControl){
 ...
 if( m_bolShowHour)  { 
  ...
  value += tiannetDateTimeSplit + hour+":00";
 }
 if( m_bolShowMinute)  { 
...
function tiannetPopCalendar(){
 ...
 var ch = document.all.divTiannetDate.clientHeight;
 var cw = document.all.divTiannetDate.clientWidth;
 var dw = document.body.clientWidth;
 var dl = document.body.scrollLeft; 
 var dt = document.body.scrollTop;

    
    if (document.body.clientHeight + dt - t - h >= ch) dads.top = (p=="image")? t + h : t + h + 6;
    else dads.top  = (t - dt < ch) ? ((p=="image")? t + h : t + h + 6) : t - ch;
    if (dw + dl - l >= cw) dads.left = l; else dads.left = (dw >= cw) ? dw - cw + dl : dl;
...
function tiannetShowControl(){
   var DivRef = document.getElementById('divTiannetDate');
...
function onclick() { 
 ...
 if (tagName != "IMG" &&tagName != "INPUT" && getAttribute("Author") != "tiannet")
    tiannetHideControl();
  }
}
