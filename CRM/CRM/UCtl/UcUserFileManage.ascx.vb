...
Partial Public Class UcUserFileManage
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If lbtext.Substring(0, 2) <> " <" Then
                If Directory.Exists(lb.CommandArgument.ToString()) Then
                    lb.Text = String.Format("<img src='../images/icons/folder.gif' style='border:none ;vertical-align:middle' /> {0}", lb.Text)

                Else
                    Dim ext As String = lb.CommandArgument.ToString().Substring(lb.CommandArgument.LastIndexOf(".") + 1)
                    If File.Exists(MyBase.Server.MapPath(String.Format("../images/icons/{0}.gif", ext))) Then
                        lb.Text = String.Format("<a href='" + lb.CommandArgument.ToString() + "'><img src='../images/icons/{0}.gif' style='border:none; vertical-align:middle;' /> {1}</a>", ext, lb.Text)
                    Else
                        lb.Text = String.Format("<a href='" + lb.CommandArgument.ToString() + "'><img src='../images/icons/other.gif' style='border:none; vertical-align:middle' /> {0}</a>", lb.Text)
    ...
    Private Sub showUserCurrPath(ByVal LondPath As String)
        ...
        For Each q In ("我的硬碟:" + getArr(1)).Split(New Char())
            ...
            comePath = comePath + q
            Me.currPath = Me.currPath + String.Format("<a id='Curpath' href='MyWebFile.aspx?path={1}'>{0}</a>", q + "\", MyBase.Server.UrlEncode(comePath))
            comePath = comePath + "\"
        Next
        ...
        If mFileClientName <> String.Empty AndAlso mFileClientName <> "" Then
            ...
            Else
                If mFileClientName <> String.Empty Then
                    ...
                    FileDownload(mFileClientName, iStream.Length, iStream)
                End If
            End If
        End If
    End Sub
