...
function GetUser(DataTableID, IsSingle) {
    $(DataTableID).datagrid({
        url: '../action/JqueryService.ashx?Method=CodeUder',
        queryParams: { UserID: '' },
        width: 600,
        height: 330,
        fitColumns: true,
        ...
        buttons: [{
            ...
            handler: function() {
                ...
                if (selected) {
                    ...
                    $('#' + UserID).val(selected.UserSID);
                    $('#' + UserNameID).val(selected.UserName);
                    $('#' + EmailID).val(selected.Email1);
                    $('#' + TelphoneID).val(selected.Telphone);
