...
var config = {
    msgurl: '/iframes/message/MyMessage.html?State=1',
    chatlogurl: '聊天记录url前缀',
    aniTime: 200,
    right: -232,
    api: {
        friend: '/resource/layim/friend.json', //好友列表接口
        group: '/resource/layim/group.json', //群组列表接口 
        chatlog: '/resource/layim/chatlog.json', //聊天记录接口
        groups: '/resource/layim/groups.json', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
        name: '游客',
    ...
    json: function(url, callback, error){
        return $.ajax({
            type: 'GET',
            url: url,
            cache: false,
...
xxim.getDates = function(index){
    ...
    config.json(api[index], function (data) {
        if(datas.status === 1){
...
//实时更新消息
xxim.interval = (function () {
    //1秒更新一次消息
    setInterval(function () {
        var msgCount = parseInt(sessionStorage.getItem("MsgCount"));
        msgCount > 0 ? $("#my_message").fadeOut(800).fadeIn(800) : $("#my_message").stop();
    },1000);
});
//渲染骨架
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'<li class="xxim_online" id="xxim_online">'
                ...
                +'</div>'
            +'</li>'
            + '<li class="xxim_mymsg" id="xxim_mymsg" title="我的消息"><i id="my_message"></i><a href="#" data-target="' + config.msgurl + '" data-model="parent" data-iframe="首页功能"></a></li>'
