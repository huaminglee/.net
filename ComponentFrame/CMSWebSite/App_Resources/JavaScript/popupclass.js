...
Object.extend(Event, {
    ...
    observe: function(element, name, observer, useCapture) {
        var element = G(element); useCapture = useCapture || false;
...
Popup.prototype = {
    ...
    build: function() {
        ...
        var close = '<img id="dialogBoxClose" src="App_Resources/Images/closewin.gif" border="0" width="28" height="15" align="absmiddle" title="关闭"/>';
        ...
        if (this.config.isHaveTitle)
            ...
            mainBox = '<div id="dialogBox" style="border:0px ;display:none;z-index:' + showZIndex + ';position:relative;width:' + this.config.width + 'px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" >';
        if (this.config.isHaveTitle) {
            mainBox += '<tr height="30" bgcolor="' + this.color.tColor + '"><td><table style="-moz-user-select:none;height:24px;" width="100%" border="0" cellpadding="0" cellspacing="0" ><tr>' + '<td width="6" height="24"></td><td id="dialogBoxTitle" style="color:' + this.color.wColor + ';font-size:12px;">' + this.info.title + '&nbsp;</td>' + '<td id="dialogClose" width="20" align="right" valign="middle">' + close + '</td><td width="6"></td></tr></table></td></tr>'
        } else {
            //mainBox += '<tr height="10"><td align="right">' + close + '</td></tr>'
            if (this.config.isShowCloseButton)
                mainBox += '<div height="1px" style="float: right; position: relative; right: 8; top: 40; z-index: 200;">' + close + '</div>'
            else
                mainBox += '<div height="1px" style="float: right; position: relative; right: 8; top: 40; z-index: 200; display:none">' + close + '</div>'
        };
        mainBox += '<tr style="height:' + this.config.height + 'px" valign="top"><td id="dialogBody" style="position:relative;"></td></tr></table></div>' + '<div id="dialogBoxShadow" style="display:none;z-index:' + baseZIndex + ';"></div>';
        if (!this.config.isBackgroundCanClick) {
            G('dialogCase').innerHTML = cover + mainBox; G('dialogBoxBG').style.height = document.body.scrollHeight
        } else
            G('dialogCase').innerHTML = mainBox; Event.observe(G('dialogBoxClose'), "click", this.reset.bindAsEventListener(this), false);
        if (this.config.isSupportDraging) {
            dropClass = new Dragdrop(this.config.width, this.config.height, this.info.shadowWidth, this.config.isSupportDraging, this.config.contentType); G("dialogBoxTitle").style.cursor = "move"
        };
        this.lastBuild()
    },
    lastBuild: function() {
        ...
        if (this.config.contentType == 1) {
            ...
            Event.observe(G('dialogOk'), "click", this.forCallback.bindAsEventListener(this), false); Event.observe(G('dialogCancel'), "click", this.close.bindAsEventListener(this), false)
    ...
    hiddenEle: function(id) {
        var ele = document.getElementById(id);
        if (typeof (ele) != "undefined" && ele != null) {
            ele.style.visibility = 'hidden'; this.someToHidden.push(ele)
        }
    },
    showSome: function() {
...
Dragdrop.prototype = {
    ...
    moveStart: function(event) {
        ...
        Event.observe(document, "selectstart", this.returnFalse, false); this.dragData = {
            x: Event.pointerX(event), y:
