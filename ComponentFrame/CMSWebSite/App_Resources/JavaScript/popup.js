       ...
       function ShowIframe(title, contentUrl, width, height) {
        ...
        function ShowIframeNoTitle(contentUrl, width, height, showCloseBtn) {

            pop = new Popup({ contentType: 1, isReloadOnClose: false, width: width, height: height, isHaveTitle: false, isShowShadow: false });
            pop.setContent("contentUrl", contentUrl);
            pop.build();
            pop.show();
        }

        function ShowIframeNoCloseButton(contentUrl, width, height) {

            pop = new Popup({ contentType: 1, isReloadOnClose: false, width: width, height: height, isHaveTitle: false, isShowShadow: false, isShowCloseButton: false });
            pop.setContent("contentUrl", contentUrl);
            pop.build();
            pop.show();
        }