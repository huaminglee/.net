...
namespace PengeSoft.CMS.Presentation
{
    ...
    public class XmlHelper
    {
        ...
        private static void SeachChild(XmlNode nodes, ref Hashtable ht)
        {
        ...
        /// <returns></returns>
        public static string GetNodeAttrib(XmlNode node, string attribName)
        {
            if (node != null && node.Attributes[attribName]!=null)
            {
                return node.Attributes[attribName].Value.ToString();
            }

            return "";
        }

        public static DataTable GetNodeList(string sPath, string sXmlNode,string ParentValue,string Value)
        {
            ...
            //载入文档
            doc.Load(sPath);
            XmlNodeList nodeList = doc.SelectNodes("//" + sXmlNode);
            ...
            if (nodeList != null && nodeList.Count>0)
            {
                if (ParentValue==null&&Value == null)
                {
                    for (int i = 0; i < nodeList.Count; i++)
                    {
                        ...
                        data.Rows.Add(GetNodeAttrib(node, "Name"), GetNodeAttrib(node, "Value"));
                    }
                }
                else if (ParentValue == null && !Value.Equals(""))
                {
                    for (int i = 0; i < nodeList.Count; i++)
                    {
                        XmlNode node = nodeList[i];
                        if (GetNodeAttrib(node.ParentNode, "Value").Equals(Value))
                        {
                            data.Rows.Add(GetNodeAttrib(node, "Name"), GetNodeAttrib(node, "Value"));
                        }
                    }
                }
                else if (!ParentValue.Equals("") && !Value.Equals(""))
                {
                    for (int i = 0; i < nodeList.Count; i++)
                    {
                        XmlNode node = nodeList[i];
                        if (GetNodeAttrib(node.ParentNode.ParentNode, "Value").Equals(ParentValue) && GetNodeAttrib(node.ParentNode, "Value").Equals(Value))
                        {
                            data.Rows.Add(GetNodeAttrib(node, "Name"), GetNodeAttrib(node, "Value"));
                        }
                    }
